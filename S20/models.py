...
class AcademicYear(BaseModel):
    ...
    name = CharField(max_length=20, unique=True, constraints=[Check("length(name) >= 4")])  # напр. "2024-2025"
...
class AcademicPeriod(BaseModel):
    ...
    start_date = DateField()
    end_date = DateField()
    order_num = IntegerField(constraints=[Check('order_num >= 1')])  # порядковый номер в году
    is_active = BooleanField(default=True)
    ...
    class Meta:
        ...
        indexes = (
            (('academic_year', 'period_type', 'order_num'), True),  # уникальная комбинация
        )
...
class PeriodGroup(BaseModel):
    """Транзитивная таблица: учебный период может охватывать несколько учебных групп,
    группа может участвовать в нескольких периодах (напр., разные специальности)"""
    id = AutoField(primary_key=True)
    period = ForeignKeyField(AcademicPeriod, backref='period_groups', on_delete='CASCADE')
    group_id = IntegerField()  # внешний ID из Group Service

    class Meta:
        table_name = 'period_groups'
        indexes = (
            (('period', 'group_id'), True),  # группа привязана к периоду только один раз
        )
...
def init_db():
    ...
    if not PeriodType.select().exists():
    ...
    if not AcademicYear.select().exists():
        ...
        sem_type = PeriodType.get(PeriodType.name == 'Семестр')
        p1 = AcademicPeriod.create(
            name='1 семестр 2024-2025',
            academic_year=year,
            period_type=sem_type,
            start_date=date(2024, 9, 1),
        ...
        # Привязка группы с id=1 к первому семестру
